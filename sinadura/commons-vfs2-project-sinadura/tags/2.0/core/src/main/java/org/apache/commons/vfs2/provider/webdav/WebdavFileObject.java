...
public class WebdavFileObject extends HttpFileObject implements FileObject
{
    ...
    private boolean isDirectory(URLFileName name) throws IOException
    {
        try
        {
            ...
            //ZYLK - para el webdav de alfresco ... supongo que habrá una forma mejor de hacerlo pero asi funciona para el upload con sinadura 
            if(property.getValue() != null && property.getValue().getClass().getName().contains("java.util.ArrayList"))
            {
            	return true;
            }
            //ZYLK
